...
class WebSocketManager:
    ...
    async def broadcast(self, channel: str, data: Dict[str, Any]) -> None:
        ...
        message = json.dumps({"channel": channel, "data": data, "ts": datetime.now(IST).isoformat()})
    ...
    async def send_personal(self, ws: WebSocket, data: Dict[str, Any]) -> None:
        ...
        if queue is None:
            return
        message = json.dumps({"channel": "personal", "data": data, "ts": datetime.now(IST).isoformat()})
        try:
            queue.put_nowait(message)
        except asyncio.QueueFull:
