...
Pipeline Steps:
  ...
  7. Hard Capital-Risk Floor: Cap quantity so that max loss ((entry - sl) x quantity)
     never exceeds 1% of total capital (independent safety floor).
...
class PositionSizer:
    """Dynamic Kelly-based position sizer with tier adjustments and a 1% hard risk floor."""
    ...
    def __init__(self, config: Dict[str, Any], capital_config: Dict[str, Any]):
        ...
        # Hard Risk Floor parameter: Max loss per trade as % of total capital (default 1.0%)
        self.hard_risk_pct: float = float(self.config.get("hard_risk_pct", 1.0))

        # Capital parameters
        self.total_capital: float = float(self.capital_config.get("virtual_capital", 100000))
    ...
    def calculate(
        ...
        **kwargs: Any,
        ...
        # 7. HARD CAPITAL-RISK FLOOR (1% Max Capital Risk per Trade)
        # Position size must NEVER risk more than hard_risk_pct (1.0%) of total capital
        # based on |entry_price - sl_price| x quantity.
        ...
        if risk_per_unit > 0 and entry_price > 0:
            ...
            if quantity > max_qty_by_risk:
                notes_parts.append(
                    f"Quantity capped from {quantity} to {max_qty_by_risk} by 1% hard capital-risk floor "
        ...
        if position_size < self.min_position_size and entry_price > 0 and quantity > 0:
            # Check if bumping to minimum position size would violate the hard 1% risk floor
            ...
            if risk_per_unit > 0 and (risk_per_unit * min_qty) > max_allowed_risk_rupees:
                notes_parts.append(
                    ...
                    f"but minimum bump suppressed to protect 1% hard risk floor"
                )
            else:
                notes_parts.append(
