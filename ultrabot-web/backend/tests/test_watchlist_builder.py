...
class MockFeed:
    ...
    async def get_candles(self, symbol: str, timeframe: str = "15m", count: int = 100):
        # Generate synthetic candles based on symbol trend
        ...
        for i in range(count):
            ...
            candles.append({
                "timestamp": f"2026-08-18T09:{i % 60:02d}:00",
                "open": price - 1.0,
                "high": price + 2.0,
                "low": price - 2.0,
                "close": price,
...
def test_scheduler_pre_market_init_populates_db():
    ...
    loop = asyncio.new_event_loop()
    try:
        loop.run_until_complete(scheduler.on_pre_market_init())
        active_items = loop.run_until_complete(repo.get_active_watchlist())
    finally:
        loop.close()

    # Confirm 10 active items populated in DB with no manual intervention
